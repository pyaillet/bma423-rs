    ...
    I2C: Write + WriteRead,
{
    ...
    fn probe_chip(&mut self) -> Result<u8, Bma423Error> {
        let mut data: [u8; 1] = [0; 1];
        self.i2c
            .write_read(self.address, &[Reg::ChipId as u8], &mut data)
            .map_err(|_| Bma423Error::I2cError)?;
        Ok(data[0])
    }

    pub fn init(&mut self) -> Result<(), Bma423Error> {
    ...
    pub fn get_status(&mut self) -> Result<u8, Bma423Error> {
        let mut data: [u8; 1] = [0; 1];
        self.i2c
